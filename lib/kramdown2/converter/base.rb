...
module Kramdown2
  ...
  module Converter
    ...
    # it in the Kramdown2::Converter module (the latter is only needed if auto-detection should work
    ...
    class Base
      ...
      #
      # 3. Append +.converter_name+ to the template name and look for it in the kramdown2 data
      ...
      def self.convert(tree, options = {})
        converter = new(tree, ::Kramdown2::Options.merge(options.merge(tree.options[:options] || {})))
        ...
        if result.respond_to?(:encode!) && result.encoding != Encoding::BINARY
          result.encode!(tree.options[:encoding] ||
                         (raise ::Kramdown2::Error, "Missing encoding option on root element"))
        end
      ...
      def self.get_template(template) # :nodoc:
        format_ext = '.' + ::Kramdown2::Utils.snake_case(self.name.split(/::/).last)
        shipped = File.join(::Kramdown2.data_dir, template + format_ext)
      ...
      def highlight_code(text, lang, type, opts = {})
        ...
        highlighter = ::Kramdown2::Converter.syntax_highlighter(@options[:syntax_highlighter])
        if highlighter
      ...
      def format_math(el, opts = {})
        ...
        engine = ::Kramdown2::Converter.math_engine(@options[:math_engine])
        if engine
      ...
      # ID.
      def generate_id(str)
        str = ::Kramdown2::Utils::Unidecoder.decode(str) if @options[:transliterated_header_ids]
      ...
      def smart_quote_entity(el)
        ...
        ::Kramdown2::Utils::Entities.entity(res)
      end
