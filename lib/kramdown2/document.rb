...
# = kramdown2
#
# kramdown2 is fast, pure Ruby Markdown superset converter, using a strict syntax definition and
...
# http://kramdown2.gettalong.org.
#
# == Usage
#
# kramdown2 has a simple API, so using kramdown2 is as easy as
#
#     require 'kramdown2'
#
#     Kramdown2::Document.new(text).to_html
#
# For detailed information have a look at the *\Kramdown2::Document* class.
#
# == License
#
...
require 'kramdown2/element'
require 'kramdown2/error'
...
module Kramdown2
  ...
  # Return the data directory for kramdown2.
  def self.data_dir
    unless defined?(@data_dir)
      require 'rbconfig'
      @data_dir = File.expand_path(File.join(File.dirname(__FILE__), '..', '..', 'data', 'kramdown2'))
      @data_dir = File.expand_path(File.join(RbConfig::CONFIG["datadir"], "kramdown2")) unless File.exist?(@data_dir)
      raise "kramdown2 data directory not found! This is a bug, please report it!" unless File.directory?(@data_dir)
    end
    @data_dir
  end

  # The main interface to kramdown2.
  #
  # This class provides a one-stop-shop for using kramdown2 to convert text into various output
  # formats. Use it like this:
  #
  #   require 'kramdown2'
  #   doc = Kramdown2::Document.new('This *is* some kramdown2 text')
  #   puts doc.to_html
  #
  ...
  class Document
    ...
    # The options hash which holds the options for parsing/converting the Kramdown2 document.
    attr_reader :options
    ...
    # Create a new Kramdown2 document from the string +source+ and use the provided +options+. The
    # options that can be used are defined in the Options module.
    #
    # The special options key :input can be used to select the parser that should parse the
    # +source+. It has to be the name of a class in the Kramdown2::Parser module. For example, to
    # select the kramdown2 parser, one would set the :input key to +Kramdown2+. If this key is not
    # set, it defaults to +Kramdown2+.
    #
    # The +source+ is immediately parsed by the selected parser so that the root element is
    # immediately available and the output can be generated.
    def initialize(source, options = {})
      @options = Options.merge(options).freeze
      parser = (@options[:input] || 'kramdown2').to_s
      parser = parser[0..0].upcase + parser[1..-1]
      try_require('parser', parser)
      if Parser.const_defined?(parser)
        @root, @warnings = Parser.const_get(parser).parse(source, @options)
      else
        raise Kramdown2::Error, "kramdown2 has no parser to handle the specified " \
          "input format: #{@options[:input]}"
      end
    end

    # Check if a method is invoked that begins with +to_+ and if so, try to instantiate a converter
    # class (i.e. a class in the Kramdown2::Converter module) and use it for converting the document.
    #
    # For example, +to_html+ would instantiate the Kramdown2::Converter::Html class.
    ...
    def try_require(type, name)
      require("kramdown2/#{type}/#{Utils.snake_case(name)}")
      true
    rescue LoadError
      true
    end
