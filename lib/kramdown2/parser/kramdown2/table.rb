...
require 'kramdown2/parser/kramdown2/block_boundary'
  ...
  module Parser
    class Kramdown2
      ...
      def parse_table
          ...
          if @src.scan(TABLE_SEP_LINE)
            ...
            @src[1].split(/(<code.*?>.*?<\/code>)/).each_with_index do |str, i|
              if i.odd?
                ...
                reset_env(src: Kramdown2::Utils::StringScanner.new(str, @src.current_line_number))
        ...
        env = save_env
        l_src = ::Kramdown2::Utils::StringScanner.new(extract_string(orig_pos...(@src.pos - 1), @src),
