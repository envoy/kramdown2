...
module Kramdown2
  module Parser
    class Kramdown2
      ...
      include Kramdown2::Parser::Html::Parser
      ...
      def parse_block_html
        ...
        if (result = @src.scan(HTML_COMMENT_RE))
          ...
          if @src.check(/^#{OPT_SPACE}#{HTML_TAG_RE}/o) && !HTML_SPAN_ELEMENTS.include?(@src[1].downcase)
            ...
            Kramdown2::Parser::Html::ElementConverter.convert(@root, @tree.children.last) if @options[:html_to_native]
            true
      ...
      def parse_span_html
        ...
        if (result = @src.scan(HTML_COMMENT_RE))
          ...
          Kramdown2::Parser::Html::ElementConverter.convert(@root, el) if @options[:html_to_native]
        else
          add_text(@src.getch)
        end
      end
