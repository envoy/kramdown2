  ...
  module Options
    ...
    def self.parse(name, data)
      ...
      unless @options[name].type === data
        ...
        data = if @options[name].type == String
                 ...
                 Integer(data) rescue raise Kramdown2::Error, "Invalid integer value for option '#{name}': '#{data}'"
               elsif @options[name].type == Float
                 Float(data) rescue raise Kramdown2::Error, "Invalid float value for option '#{name}': '#{data}'"
    ...
    def self.simple_array_validator(val, name, size = nil)
      if String === val
        ...
        raise Kramdown2::Error, "Invalid type #{val.class} for option #{name}"
      end
      if size && val.size != size
        raise Kramdown2::Error, "Option #{name} needs exactly #{size} values"
      end
      val
    end
    ...
    def self.simple_hash_validator(val, name)
      if String === val
        begin
          ...
          raise Kramdown2::Error, "Invalid YAML value for option #{name}"
        end
      end
      raise Kramdown2::Error, "Invalid type #{val.class} for option #{name}" unless Hash === val
      val
    end
    ...
    define(:template, String, '', <<~EOF)
      ...
      kramdown2 (without the converter extension). If the file is still not
      ...
      kramdown2 provides a default template named 'document' for each converter.
    ...
    define(:auto_id_prefix, String, '', <<~EOF)
      ...
      header IDs so that there is no conflict when rendering multiple kramdown2
    ...
    define(:parse_block_html, Boolean, false, <<~EOF)
      Process kramdown2 syntax in block HTML tags

      If this option is `true`, the kramdown2 parser processes the content of
      block HTML tags as text containing block-level elements. Since this is
      not wanted normally, the default is `false`. It is normally better to
      selectively enable kramdown2 processing via the markdown attribute.
    ...
    define(:parse_span_html, Boolean, true, <<~EOF)
      Process kramdown2 syntax in span HTML tags
    ...
    EOF
      ...
      val.each do |_k, v|
        if !(Array === v) || v.size > 2 || v.empty?
          raise Kramdown2::Error, "Invalid structure for hash value of option #{name}"
        end
        v << nil if v.size == 1
      end
      val
    ...
    define(:entity_output, Symbol, :as_char, <<~EOF)
      ...
      Default: :as_char
      Used by: HTML converter, kramdown2 converter
    EOF
    ...
    EOF
      ...
      when String
        if val =~ /^(\d)\.\.(\d)$/
          ...
          raise Kramdown2::Error, "Invalid syntax for option toc_levels"
        end
      when Array
      ...
      when Range
        ...
        raise Kramdown2::Error, "Invalid type #{val.class} for option toc_levels"
      end
      if val.any? {|i| !TOC_LEVELS_RANGE.cover?(i) }
        raise Kramdown2::Error, "Level numbers for option toc_levels have to be integers from 1 to 6"
      end
      val
    end
    ...
    define(:header_offset, Integer, 0, <<~EOF)
      ...
      Default: 0
      Used by: HTML converter, Kramdown2 converter, Latex converter
    EOF
